...
namespace Telegent
{
    class Program
    {
        ...
        private static TelegramClient client = new TelegramClient(TLAPIData.apiId, TLAPIData.apiHash);
        ...
        private static bool RequestAuthCode(string phone, out int intAuthCode, out string hash)
        {
            ...
            string strAuthCode = "";
            Logger.Warn("Telegram Auth Code required, please enter 5 digits: ");
        ...
        private static bool SaveLogToFile(string file, string lines, string entity, string source)
        {
            if (null != file)
            {
                File.WriteAllLines(file, lines
                    .Insert(0, "[Title: " + entity + " | Source: " + source + " | Count: " + (lines.Split(Environment.NewLine).Count()-1).ToString() + "]" + Environment.NewLine)
